...
from positionalen import get_positional_encoding
...
class EmbeddingsWithLearnedPositionalEncoding(Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return self.linear(x) * math.sqrt(self.d_model) + pe
#TRANSFORMER LAYER acting as an encoder and a decoder
...
#keys,values and query are from self
        self_attn = self.self_attn(query=z, key=z, value=z, mask=mask)
#add self attention result
        x= x+self.dropout(self_attn)
#If a source is provided, get results from attention to source. This is when you have a decoder layer
# that pays attention to encoder outputs
        if src is not None:
            z=self.norm_self_attn
#Attention to source (keys and values from source)
            attn_src=self.src_attn(query=z, key=src, value=src, mask=src_mask)
#add the source attention results
            x=x +self.dropout(attn_src)
        #Normalizing for feed forward
        z=self.norm_ff(x)
...
#TRANSFORMER ENCODERRR
class Encoder(Module):
    def __init__(self, layer: TransformerLayer, n_layers: int):
        super().__init__()
#here we will make copiens of the transformer layer
